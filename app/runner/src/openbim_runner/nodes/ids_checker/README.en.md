...
| `generate_detailed_report` | `bool` (default: `false`) | When enabled, results are additionally grouped by specification (for report generation). Combined lists are always created. Requires references from a single model. |
...
- `specifications: list[SpecificationResult] | null` — Per-specification breakdown (its per-spec lists also hold qualified references). Only included when `generate_detailed_report` is enabled. Omitted from output when disabled.
